...
func (m *Manager) Secrets(kind string) map[string]string {
	...
	case KindClaude:
		switch row.Method {
		case MethodToken, MethodSubscription:
...
func (m *Manager) driveLogin(ctx context.Context, login *Login, bin string) {
	...
	switch login.Kind {
	case KindClaude:
		// Bare `claude` triggers the login prompt on first run.
		args = append(args, bin)
	case KindCodex:
		args = append(args, bin, "login")
	}
...
func (m *Manager) finishOK(ctx context.Context, login *Login) {
	// Read the credential file and seal in the vault so future sessions can
	// inject env vars without re-parsing the file on every start.
