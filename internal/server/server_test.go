...
// Phase 9 AC1: a replayed request id (same id sent twice on the same
// connection) does not re-execute the side-effect; the second arrival is
// answered with a replay marker instead.
func TestServer_DedupesReplayedFrameID(t *testing.T) {
	dir := t.TempDir()
	...
	// Replay with the same id; should be answered without creating a second
	// project.
	if err := c.Write(ctx, websocket.MessageText, req); err != nil {
		t.Fatal(err)
	}
	...
	// List shows exactly one project.
